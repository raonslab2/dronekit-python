...
if __name__ == "__main__":
    ...
    # Load configuration from drone_config.json
    config = read_config('drone_config.json')

    drone_info = DroneState()
    host = config.get('host', "ws://13.209.238.3:5010/websocket")
    drone_host = config.get('drone_host', "lm_10001")
    receive_port = config.get('receive_port', "14540")

    db_config = config.get('database', {})
    db_host = db_config.get('host', 'localhost')
    db_user = db_config.get('user', 'root')
    db_password = db_config.get('password', '')
    db_database = db_config.get('database', 'test')
